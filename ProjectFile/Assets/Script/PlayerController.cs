...
public class PlayerController : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            wallAnimator = collision.gameObject.GetComponent<Animator>();
            isAnimationPlaying = wallAnimator.GetCurrentAnimatorStateInfo(0).IsName("MidTrigger");
            // Trigger the wall animation if the wall has an Animator.
            if (wallAnimator != null && !isAnimationPlaying)
            {
                wallAnimator.SetTrigger("Bounce");
            }
        }
    }
